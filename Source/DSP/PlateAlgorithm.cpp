...
namespace bloomverb
{
void PlateAlgorithm::prepare(double sampleRate)
{
    fdnCore = std::make_unique<FDNCore>();
    fdnCore->prepare(sampleRate);
}

void PlateAlgorithm::reset()
{
    if (fdnCore)
        fdnCore->reset();
}

void PlateAlgorithm::processSample(float diffuseL,
                                  float diffuseR,
                                  ...
                                  float& wetR)
{
    ...
    fdnSettings.size = juce::jlimit(0.0f, 1.0f, (smoothed.size * typeSizeScale + ctx.decayRoomBias) * 0.82f);
    fdnSettings.decaySeconds = smoothed.decaySeconds;
    fdnSettings.damping = juce::jlimit(0.0f, 1.0f, smoothed.damping + profile.dampingBias + ctx.decayDampingBias + 0.18f);
    ...
    fdnSettings.harmonic = juce::jlimit(0.0f, 1.0f, smoothed.harmonic * harmonicTypeScale + 0.22f);
    ...
    fdnSettings.roomScale = juce::jlimit(0.55f, 1.70f, (profile.roomScale + ctx.decayRoomBias) * 0.68f);
