...
namespace bloomverb
{
void CloudBloomDreamAlgorithm::prepare(double sampleRate)
{
    fdnCore = std::make_unique<FDNCore>();
    fdnCore->prepare(sampleRate);
}

void CloudBloomDreamAlgorithm::reset()
{
    if (fdnCore)
        fdnCore->reset();
}

void CloudBloomDreamAlgorithm::processSample(float diffuseL,
                                            float diffuseR,
                                            ...
                                            float& wetR)
{
    ...
    fdnSettings.size = juce::jlimit(0.0f, 1.0f, (smoothed.size * typeSizeScale + ctx.decayRoomBias) * 1.22f);
    fdnSettings.decaySeconds = smoothed.decaySeconds * 1.15f;
    fdnSettings.damping = juce::jlimit(0.0f, 1.0f, smoothed.damping + profile.dampingBias + ctx.decayDampingBias - 0.10f);
    fdnSettings.diffusion = juce::jlimit(0.0f, 1.0f, diffusionAmount + 0.18f);
    fdnSettings.width = juce::jlimit(0.0f, 2.5f,
                                     smoothed.width * profile.widthScale * 1.28f
