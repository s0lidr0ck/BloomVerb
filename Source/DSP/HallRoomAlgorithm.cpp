...
namespace bloomverb
{
void HallRoomAlgorithm::prepare(double sampleRate)
{
    fdnCore = std::make_unique<FDNCore>();
    fdnCore->prepare(sampleRate);
}

void HallRoomAlgorithm::reset()
{
    if (fdnCore)
        fdnCore->reset();
}

void HallRoomAlgorithm::processSample(float diffuseL,
                                     float diffuseR,
                                     ...
                                     float& wetR)
{
    ...
    fdnCore->processSample(diffuseL,
                          diffuseR,
                          fdnSettings,
                          wetL,
                          wetR);
